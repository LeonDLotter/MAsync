# ========================================================================
# Figure 2 - Brains & Functional Context
# ========================================================================
...
# plot
fig2ab = plt.figure(figsize=(11,12.5), constrained_layout=False)
gs = fig2ab.add_gridspec(ncols=1, nrows=4, hspace=0)
# Row 1: ale z
ax1 = fig2ab.add_subplot(gs[0,0])
plot_ale_z = plot_glass_brain(ale_z, figure=fig2ab, axes=ax1, display_mode='lyrz',
                            title='ALE (no threshold)', cmap=c_continous)
# Row 2: ale cl, p < 0.001
ax2 = fig2ab.add_subplot(gs[1,0])
plot_ale01 = plot_glass_brain(ale_cl001, figure=fig2ab, axes=ax2, display_mode='lyrz',
                              title='ALE (p < .001)', cmap=c_ale001)
plot_ale01.add_overlay(loeo_cl001, cmap=c_overlay)
# Row 3: ale cl, p < 0.01
ax3 = fig2ab.add_subplot(gs[2,0])
plot_ale02 = plot_glass_brain(ale_cl01, figure=fig2ab, axes=ax3, display_mode='lyrz',
                              title='ALE (p < .01)', cmap=c_ale01)
plot_ale02.add_overlay(loeo_cl01, cmap=c_overlay)
# Row 4: macm
ax1 = fig2ab.add_subplot(gs[3,0])
plot_macm = plot_glass_brain(macm_idx, figure=fig2ab, axes=ax1, display_mode='lyrz',
                             title='MACM', cmap=c_macm, vmin=0)
plot_macm.add_contours(math_img('i*(i<2)',i=ale_cl001), colors='k', linewidths=0.7)                            

plt.savefig(join(sdir, 'fig2ab.pdf'), transparent=True, bbox_inches='tight')
...
plt.savefig(join(sdir, 'fig2c1.pdf'), transparent=True, bbox_inches='tight')
...
plt.savefig(join(sdir, 'fig2c2.pdf'), transparent=True, bbox_inches='tight') 
...
map_lh, map_rh = mni152_to_fslr(fnirs, method='nearest')

p = Plot(surf_lh=lh, surf_rh=rh, layout='row', size=(1000, 300), zoom=1.2, 
         views=['lateral', 'medial'], flip=False)
p.add_layer({'left': sulc_lh, 'right': sulc_rh}, cmap='binary_r', cbar=False)
p.add_layer({'left': map_lh, 'right': map_rh}, cmap='inferno', cbar=True)

outline_lh, outline_rh = mni152_to_fslr(macm_idx, method='nearest')
p.add_layer({'left': outline_lh, 'right': outline_rh}, 
             cmap=c_ale001, as_outline=True, cbar=False)

kws = dict(location='bottom', draw_border=True, aspect=10, shrink=.2,
           decimals=0, pad=-0.13)
fig2c = p.build(cbar_kws=kws) 

plt.savefig(join(sdir, 'fig2d.pdf'), dpi=400, bbox_inches='tight', transparent='true')
...
plt.savefig('fig2e.pdf', transparent=True, bbox_inches='tight')
...
plt.savefig('fig2d.pdf', transparent=True, bbox_inches='tight')
