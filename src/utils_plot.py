...
def plot_surf(vol, title='', interp='linear', legend=True, cmap='viridis',
    ...
    map_lh, map_rh = mni152_to_fslr(vol, method=interp)

    p = Plot(surf_lh=lh, surf_rh=rh, layout='row', size=size, zoom=zoom, 
             views=views, flip=flip)
    p.add_layer({'left': sulc_lh, 'right': sulc_rh}, cmap='binary_r', cbar=False)
    p.add_layer({'left': map_lh, 'right': map_rh}, cmap=cmap, cbar=legend)
    
    if overlay is not None:
        outline_lh, outline_rh = mni152_to_fslr(overlay, method='nearest')
